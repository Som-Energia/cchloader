...
class ReganecuSchema(Schema):
    ...
    reserved1 = fields.Integer(position=4, allow_none=True)
    precio = fields.Float(position=5, required=True)
    reserved2 = fields.Integer(position=6, allow_none=True)
    importe = fields.Float(position=7, required=True)
    reserved3 = fields.Integer(position=8, allow_none=True)
